...
//    leaderBoardInfo A("ngobang", "99999", "27/11/2023");
    ...
    for (int i = 0; i < info.size(); i++) {
        ...
        b.push_back(new Text(mainFrame, info[i].getScore()));
        c.push_back(new Text(mainFrame, info[i].getDate()));
        a[i]->writeText(getScoreBoardCoord(a[i]->getLength(), NAME_COL), ROW_OFFSET[i]);
        b[i]->writeText(getScoreBoardCoord(b[i]->getLength(), SCORE_COL), ROW_OFFSET[i]);
        c[i]->writeText(getScoreBoardCoord(c[i]->getLength(), DATE_COL), ROW_OFFSET[i]);
    }
...
void MenuScreen::screenGameOver(const int &score, const string &name) {
    ...
    stable_sort(info.begin(), info.end(), [](const leaderBoardInfo &a, const leaderBoardInfo &b) {
